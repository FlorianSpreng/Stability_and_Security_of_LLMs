...
def get_patient_system(persona: str, lang: str) -> str:
    config = configparser.ConfigParser()
    with open(
            os.path.join(f"data_{lang}", "patients", f"{persona}_config.ini"), "r", encoding="utf-8"
    ) as f:
        config.read_file(f)
    ...
    system_prompt = (
        f"{open(os.path.join('.', f'data_{lang}', 'patients', 'pre_prompt.txt'), 'r').read()}\n"
        f"{config['Records']['text']}\n"
        f"{diagnose_string}\n"
        f"You have the following personality: {config['Persoenlichkeit']['text']}\n"
        f"{open(os.path.join('.', f'data_{lang}', 'patients', 'supplemental_prompt.txt'), 'r', encoding='utf-8').read()}\n"
    )
...
def get_doctor_system(medic: str, lang: str) -> str:
    config = configparser.ConfigParser()
    with open(os.path.join('.', f'data_{lang}', 'medics', f'{medic}_config.ini'), "r", encoding="utf-8") as f:
        config.read_file(f)
    ...
    system_prompt = (
        f"{open(os.path.join('.', f'data_{lang}', 'medics', 'pre_prompt.txt'), 'r', encoding='utf-8').read()}\n"
        f"{user_data_string}"
        f"{open(os.path.join('.', f'data_{lang}', 'medics', 'supplemental_prompt.txt'), 'r', encoding='utf-8').read()}\n"
    )
